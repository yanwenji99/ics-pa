...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(rd) = src1 + imm; printf("li\n"););
  // INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi, I, R(rd) = R(rs1) + imm);
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui, U, R(rd) = imm; printf("li\n"););
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , I, s->dnpc = s->pc + imm; R(rd) = s->pc + 4;);
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2););
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + imm; R(rd) = s->pc + 4;printf("ra = %x\n", R(1)););
